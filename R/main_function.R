...
gKRLS <- function(
  ...
  std_design <- standardize_design(kernel_X = kernel_X, standardize = standardize)
  std_kernel_X <- std_design$std_kernel_X
  std_train <- std_design$std_train
  rm(std_design); gc()
  ...
  if (sketch_method == 'nystrom'){
    ...
    S <- create_sketch_matrix(N, sketch_size, sketch_prob, sketch_method)
  }
