...
calculate_effects <- function(model, data = NULL, 
  ...
  if (is.null(data)){
    raw_data <- prediction::find_data(model)
  }else{
    raw_data <- data
    rm(data)
  }
  ...
  all_variables <- find_terms_in_model.gam(model = model, variables = NULL)
  ...
  any_binary <- apply(find_data(model)[,names(variable_type)], 
                      MARGIN = 2, 
  ...
  for (cond_i in seq_len(ncond)){
    if (cond_i %% ceiling(ncond/ncond) == 0){
      cat('|')
    }
    data <- raw_data
